...
NOME_ARQUIVO_CONFIG = "config_pista_14S_Aruja_V6.txt"
...
def carregar_configuracoes(caminho_arquivo_txt):
    ...
    try:
        ...
        config['COORDENADAS_CROP'] = (
            int(temp_config['CROP_X1']), int(temp_config['CROP_Y1']),
            int(temp_config['CROP_X2']), int(temp_config['CROP_Y2'])
        )

        config['PERFIL_NORMAL'] = {
            'shutter': int(temp_config['PERFIL_NORMAL_SHUTTER']),
            'gamma': int(temp_config['PERFIL_NORMAL_GAMMA'])
        }
        config['PERFIL_SOL_MEDIO'] = {
            'shutter': int(temp_config['PERFIL_SOL_MEDIO_SHUTTER']),
            'gamma': int(temp_config['PERFIL_SOL_MEDIO_GAMMA'])
        }
        config['PERFIL_SOL_FORTE'] = {
            'shutter': int(temp_config['PERFIL_SOL_FORTE_SHUTTER']),
            'gamma': int(temp_config['PERFIL_SOL_FORTE_GAMMA'])
        }
        config['PERFIL_RETORNO'] = {
            'shutter': int(temp_config['PERFIL_RETORNO_SHUTTER']),
            'gamma': int(temp_config['PERFIL_RETORNO_GAMMA'])
        }
        ...
        config['DIRETORIO_IMAGENS_ALERTA'] = temp_config.get('DIRETORIO_IMAGENS_ALERTA', DIRETORIO_IMAGENS_ALERTA_BASE)
        config['SALVAR_IMAGEM_CROPADA_DEBUG'] = temp_config['SALVAR_IMAGEM_CROPADA_DEBUG'].lower() == 'true'
        config['DIRETORIO_IMAGENS_CROPADAS'] = temp_config.get('DIRETORIO_IMAGENS_CROPADAS', DIRETORIO_IMAGENS_CROPADAS_BASE)

        config['CLP_PORT'] = 502
        config['ENDERECO_CANCELA'] = 226

        # --- NOVO: regra de baixo ganho no NORMAL (com defaults para não quebrar se não existir no TXT) ---
        config['LIMIAR_GANHO_NORMAL_BAIXO'] = int(temp_config.get('LIMIAR_GANHO_NORMAL_BAIXO', '6'))
        config['PERFIL_NORMAL_BAIXO_GANHO'] = {
            'shutter': int(temp_config.get('PERFIL_NORMAL_BAIXO_GANHO_SHUTTER', '100')),
            'gamma': int(temp_config.get('PERFIL_NORMAL_BAIXO_GANHO_GAMMA', '30')),
        }
...
def analisar_qualidade_imagem_camera(ip_camera):
    ...
    caminho_base = encontrar_caminho_recurso('')
    diretorio_imagens_path = os.path.join(
        caminho_base,
        config.get('DIRETORIO_IMAGENS_ALERTA', DIRETORIO_IMAGENS_ALERTA_BASE)
    )
    diretorio_crop_path = os.path.join(
        caminho_base,
    ...
    try:
        ...
        with Image.open(io.BytesIO(image_data)) as img:
            ...
            if percentual_estourado > config['LIMIAR_ESTOURO_ALERTA_INICIAL'] and config.get('SALVAR_IMAGEM_NO_ALERTA', False):
                try:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if match:
                ...
                log_message(
                    f"[INFO GANHO] Resposta: '{response.text.strip()}'. Ganho extraído: {ganho}",
...
def ajustar_camera(perfil):
    ip = config['CAMERA_IP']
    perfil_map = {
        'normal': config['PERFIL_NORMAL'],
        'sol_medio': config['PERFIL_SOL_MEDIO'],
        'sol_forte': config['PERFIL_SOL_FORTE'],
        'retorno': config['PERFIL_RETORNO'],
    }
    if perfil not in perfil_map:
        return False
...
# --- Função: ajuste direto por valores (usada no perfil de normal com ganho baixo) ---
def ajustar_camera_valores(shutter, gamma, motivo="custom"):
    ip = config['CAMERA_IP']
    print(f"Ajustando câmera ({motivo}) -> Shutter: {shutter}, Gamma: {gamma}")
    log_message(f"[AÇÃO] Ajuste direto ({motivo}) -> Shutter:{shutter}, Gamma:{gamma}", config['NOME_PISTA'])
    comando_string = f"ShutterMaximo={shutter}&ValorGammaDif={gamma}"
    url_comando = f"http://{ip}/api/config.cgi?{comando_string}"
    try:
        response = requests.get(url_comando, timeout=10)
        return response.status_code == 200
    except Exception as e:
        log_message(f"[ERRO AJUSTE DIRETO] {e}", config['NOME_PISTA'])
        return False
...
def main():
    ...
    # flag para não reenviar ajuste de baixo ganho a cada gatilho
    ajuste_baixo_ganho_ativo = False
    ...
    prev_cancela_status = False
    print("Monitorando sinal da cancela...")

    try:
        while True:
            if datetime.now().time() > config['HORA_ENCERRAMENTO']:
                print("Horário de encerramento atingido.")
                log_message("Horário de encerramento atingido.", nome_pista)
                break

            try:
                rr = client.read_coils(address=config['ENDERECO_CANCELA'], count=1)
                if rr.isError():
                    rr = client.read_discrete_inputs(address=config['ENDERECO_CANCELA'], count=1)
                    if rr.isError():
                        raise Exception("Erro Modbus.")

                current_cancela_status = rr.bits[0]

                if current_cancela_status and not prev_cancela_status:
                    log_message("[GATILHO]", nome_pista)

                    metricas = analisar_qualidade_imagem_camera(config['CAMERA_IP'])
                    ganho_atual = obter_ganho_atual()
                    if metricas is None or ganho_atual is None:
                        prev_cancela_status = current_cancela_status
                        continue
                    ...
                    if estado_atual == 'normal':
                        # --- NORMAL: regra de ganho baixo (valores vêm do TXT) ---
                        limiar_baixo = config['LIMIAR_GANHO_NORMAL_BAIXO']
                        perfil_bg = config['PERFIL_NORMAL_BAIXO_GANHO']

                        if ganho_atual < limiar_baixo:
                            if not ajuste_baixo_ganho_ativo:
                                ok = ajustar_camera_valores(
                                    perfil_bg['shutter'],
                                    perfil_bg['gamma'],
                                    motivo=f"normal_ganho<{limiar_baixo}"
                                )
                                if ok:
                                    ajuste_baixo_ganho_ativo = True

                            # Se quiser PRIORIDADE TOTAL e não permitir escalar no mesmo gatilho, descomente:
                            # prev_cancela_status = current_cancela_status
                            # continue
                        else:
                            if ajuste_baixo_ganho_ativo:
                                ok = ajustar_camera('normal')
                                if ok:
                                    ajuste_baixo_ganho_ativo = False
                        ...
                        if (cond_estouro and cond_brilho_forte and cond_ganho_forte) or \
                           (cond_estouro and cond_brilho_medio and cond_ganho_medio):
                            ...
                            if contador_para_escalar >= config['CONTADOR_PARA_ESCALAR']:
                                ...
                                if ajustar_camera(novo_estado):
                                    estado_atual = novo_estado
                                    # ao sair do normal, zera o ajuste especial
                                    ajuste_baixo_ganho_ativo = False
                        ...
                        if estouro > config['LIMIAR_ESTOURO_ESCALADA_MEDIA']:
                            contador_para_escalar += 1
                            if contador_para_escalar >= config['CONTADOR_PARA_ESCALAR']:
                                if ajustar_camera('sol_forte'):
                                    estado_atual = 'sol_forte'
                                contador_para_escalar, contador_para_desescalar = 0, 0
                        else:
                            contador_para_escalar = 0

                        if estouro < config['LIMIAR_ESTOURO_OK'] and brilho_medio < config['LIMIAR_BRILHO_MEDIO_PARA_NORMAL']:
                            contador_para_desescalar += 1
                            if contador_para_desescalar >= config['CONTADOR_PARA_DESESCALAR']:
                                if ajustar_camera('normal'):
                                    estado_atual = 'normal'
                                contador_para_escalar, contador_para_desescalar = 0, 0
                        else:
                        ...
                        cond_retorno_normal = (estouro < config['LIMIAR_ESTOURO_OK'] and
                                              brilho_medio < config['LIMIAR_BRILHO_MEDIO_PARA_NORMAL'])
                        cond_retorno_medio = (estouro < config['LIMIAR_ESTOURO_OK'] and
                                             brilho_medio < config['LIMIAR_BRILHO_MEDIO_PARA_MEDIO'])
        ...
        ajustar_camera('retorno')
        log_message("Script encerrado. Câmera configurada para o modo 'retorno'.", nome_pista)
